...
router.get('/loadPinyear', (req, res) => {
    res.header('Access-Control-Allow-Origin', '*'); // Allow all origins, or specify a specific origin
    res.header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS'); // Allow specified methods
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept'); // Allow specified headers
    ...
    pool.getConnection((err, connection) => {
        ...
        query = 'SELECT ac_session,sessionID FROM academicsession '
        connection.query(query, (error, results) => {
            if (error) {
                connection.release()
                console.log(error)
                ...
                if (results.length > 0) {
                    ...
                    return res.status(201).json({ message: 'Academic years are not mounted yet!' });
                }
            }
        })
    })
        ...
        connection.query(query, (error, results) => {
            if (error) {
                ...
                if (results.length > 0) {
                    ...
                    connection.release()
                    console.log('not mounted yet')
                    return res.status(201).json({ message: 'Grades are not mounted yet!' });
                }
            }
        })
    })
...
router.post('/submitPins', (req, res) => {
    const data = req.body
    res.header('Access-Control-Allow-Origin', '*'); // Allow all origins, or specify a specific origin
    res.header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS'); // Allow specified methods
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept'); // Allow specified headers
    ...
    pool.getConnection((err, connection) => {
        ...
        const values = data.map(item => [
            ...
            item.SerialPin,
            item.dateGenerated,
            item.yearId // Fixed typo from academic_year to match your data object key
        ]);
        const query = "INSERT INTO applicationserial (SerialNumber, SerialPin, dateGenerated, academic_year) VALUES ?";
        connection.query(query, [values], (error, results) => {
            if (error) {
                ...
                if (results.affectedRows > 0) {
                    connection.release()
                    return res.status(200).json({ success: 'A total of ' + data.length + ' have successfuly been summited' })
                } else {
                    connection.release()
...
router.get('/get-total-forms-generated', (req, res) => {
    res.header('Access-Control-Allow-Origin', '*'); // Allow all origins, or specify a specific origin
    res.header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS'); // Allow specified methods
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept'); // Allow specified headers
    ...
    pool.getConnection((err, connection) => {
        ...
        query = 'SELECT academic_year,  COUNT(SerialNumber) AS total FROM applicationserial GROUP BY academic_year ORDER BY academic_year DESC '
        connection.query(query, (error, results) => {
            if (error) {
                connection.release()
                console.log(error)
                ...
                if (results.length > 0) {
                    connection.release()
                    const total = results
                    query = 'SELECT  COUNT(SerialNumber) AS total FROM salesofapplicationserial GROUP BY academic_year ORDER BY academic_year DESC '
                    connection.query(query, (error, results) => {
                        if (error) {
                            connection.release()
                            console.log(error)
                           ...
                           const combinedConcat=total.concat(sold)
                                    
                         
                           
                                console.log(combinedConcat)
                                           connection.release()
                                return res.status(200).json({ data: combinedConcat })
                            } else {
                                connection.release()
                                console.log('no sales')
                                return res.status(201).json({ message: 'Nothing has been sold yet' });
                            }
                    ...
                    connection.release()
                    console.log('not mounted yet')
                    return res.status(201).json({ message: 'Application forms are not yet mounted' });
                }
            }
        })
    })
